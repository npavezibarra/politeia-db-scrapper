...
BASE_URL = "https://www.emol.com/especiales/2024/nacional/elecciones2024/resultados.asp"
OUTPUT_DIR = "/Users/nicolas/Desktop/PoliteiaDB/I"

# Keeping the full map for safety, though only Region I codes will be used effectively
COMMUNE_CODES = {
...
async def extract_tarapaca_communes(page):
    """
    Extracts (Commune Name, Emol ID) tuples for Region I (Tarapacá).
    """
    print("Strategy: Extracting Tarapacá communes list first...")
    ...
    for reg in regions:
        ...
        # Fix: handle lowercase and 'I'
        if r_code and (r_code.lower() == "i" or r_code.lower() == "1"):
            print(f"Found Tarapacá block (code: {r_code})")
            items = await reg.query_selector_all("li[data-el='a']")
            for li in items:
                name = (await li.text_content()).strip()
                code = await li.get_attribute("data-zn")
                if name and code:
                     communes.append((name, code))
            found_region = True
            break
            
    # No fallback for now, as we expect "i" to exist
    print(f"Extracted {len(communes)} communes for Tarapacá.")
    return communes
...
async def scrape_to_csv():
    ...
    # Ensure Tarapacá region exists
    region_id = csv_gen.get_or_create_jurisdiction("Región de Tarapacá", "REGION")
    ...
    async with async_playwright() as p:
        ...
        # 1. Discovery Phase
        communes = await extract_tarapaca_communes(page)
        ...
        for idx, (commune_name, emol_id) in enumerate(communes):
            ...
            for attempt in range(retries):
                try:
                    ...
                    if attempt == 0:
                        ...
                        # NEW: Explicitly Click "Alcaldes" Tab (found by subagent to be useful)
                        try:
                            # Selector guessed based on standard UI, or by text
                            alcaldes_tab = await page.query_selector("li >> text=ALCALDES")
                            if alcaldes_tab:
                                await alcaldes_tab.click()
                                await page.wait_for_timeout(1000)
                        except Exception as tab_e:
                            print(f"  Warning: Could not click Alcaldes tab: {tab_e}")

                        # NEW: Click Region Header First (essential for Region I)
                        try:
                            # We blindly try to click the Region I header "data-region='i'"
                            # Logic: If it exists, click it to expand.
                            # Note: RM scraper didn't need this because RM is default.
                            # We can make this generic or specific to this script.
                            # Selector: li[data-el='g'][data-region='i']
                            # We use query_selector to avoid error if not found (though it should be there)
                            r_header = await page.query_selector("li[data-el='g'][data-region='i']")
                            if r_header:
                                await r_header.click()
                                await page.wait_for_timeout(1500) # Wait for accordion
                        except Exception as rh_e:
                             print(f"  Warning: Could not click Region I header: {rh_e}")
                    ...
                    try:
                        ...
                        await page.screenshot(path=os.path.join(OUTPUT_DIR, "screenshots", f"debug_fail_{commune_name}.png"))
            ...
            try:
                # Commune Entity
                commune_id = csv_gen.get_or_create_jurisdiction(commune_name, "COMMUNE", region_id)
                election_id = csv_gen.create_election(OFFICE_ALCALDE, commune_id, commune_name)
                
                # Candidates - Wait for visibility as per user instruction
                try:
                    # Increased timeout to 20s
                    await page.wait_for_selector("ul.res-ul-candidatos", state="visible", timeout=20000)
                except:
                    print(f"  Warning: Candidate list did not become visible for {commune_name}")
                
                # Take Screenshot (Fallback)
                try:
                    box = await page.query_selector("div.res-box-content")
                    if box:
                        sanitized_name = commune_name.replace(" ", "_").lower()
                        ss_path = os.path.join(OUTPUT_DIR, "screenshots", f"{sanitized_name}.png")
                        await box.screenshot(path=ss_path)
